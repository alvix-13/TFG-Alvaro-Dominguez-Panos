...
def conectar_servidor():
    ...
    try:
        ...
        conectate.config(text=f"Conectado a {local_host} con éxito")
    except:
        conectate.config(text=f"Error al conectar con {local_host}")

    t1 = threading.Thread(target=mandar_prediccion)
...
def mandar_prediccion(prediccion):
    sock.sendall(prediccion.encode())
...
conectate = tk.Label(ventana, text="Introduce la IP del host:", font=("Helvetica", 12, "bold"), bg="gray", fg="white")
conectate.grid(row=1, column=0, pady=15, padx=15)

ip = tk.Entry(ventana)
ip.grid(row=1, column=1, pady=15, padx=15)

boton_conectar = tk.Button(ventana, text="Conectar", command=conectar_servidor, font=("Helvetica", 12, "bold"), bg="black", fg="white")
...
boton_conectar.grid(row=1, column=2, pady=15, padx=15)
boton_ejecutar.grid(row=2, column=0, pady=15, padx=15)
boton_detener.grid(row=4, column=0, pady=15, padx=15)
boton_entrenar.grid(row=2, column=2, pady=15, padx=15)
boton_entrenar_rapido.grid(row=4, column=2, pady=15, padx=15)
